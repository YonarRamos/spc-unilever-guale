...
export default {
  ...
  methods: {
    graficar() {
      if (!this.chart) {
        this.chart = echarts.init(document.getElementById('grafico_tv4'))
      }
    ...
    async getDetalleTendenciaTv(producto) {
      ...
      await axios
        ...
        .then(response => {
          this.$store.commit('socket/SET_DETALLE_TENDENCIA_TV4', response.data)
        })
    }
  }
}
